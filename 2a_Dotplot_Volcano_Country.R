...
testdata %>%
    mutate(
           col_or = ifelse(OR>1, 'Enriched in country',
                           'Depleted in country'),
...
# Exported 1000 x 500 (age_diag)


# Dot plot
bonf_across_countries = testdata %>%
    group_by(signature) %>%
    summarise(bonf_all_countries = sum(q_val_2<0.05))

testdata$vartype = factor(testdata$vartype, levels = c('SB', 'DB', 'ID',
                                                       'CN','SV'))
...
met2 = df
met2$country = as.character(met2$country)
met2$country[met2$country=='Czech Republic'] = 'Czechia'
if (variable == 'country'){
    country_order = met2 %>%
        group_by(country) %>%
...
if (variable == 'tumorsite_group'){
    country_order = c('Distal colon', 'Rectum')
}

levels(testdata$country)[levels(testdata$country)=="Czech Republic"] = "Czechia"

testdata %>%
    left_join(bonf_across_countries) %>%
    mutate(signature = factor(signature,
                              levels = gtools::mixedsort(unique(signature),
                                                         decreasing = T)),
    ...
    mutate(vartype = str_replace(vartype, 'SB', 'SBS'),
           ...
           vartype = factor(vartype, levels = c('SBS',
                                                ...
                                                'SV'))) %>%
ggplot() +
    aes(y = reorder(country, desc(country)),
        x = reorder(signature, desc(signature)),
        size = -log10(q_val_2), fill = log2(OR),
        col = (q_val_2<0.05)) +
    facet_grid(.~vartype, scales = 'free', space = 'free') +
    xlab(NULL) +
    geom_point(shape = 21, stroke = 1.2, ) +
    scale_color_manual(values = c('transparent', 'red')) +
    scale_fill_distiller(type = 'div', palette = 4,
                         # limits = c(-max(abs(c(max(log2(testdata$OR)),
                         #                       min(log2(testdata$OR[testdata$OR>0]))))
                         #                 ),
                         #            +max(abs(c(max(log2(testdata$OR)),
                         #                       min(log2(testdata$OR[testdata$OR>0]))))
                         #                 )
                         #            )
                         limits = c(-5,5)
                         ) +
    scale_size_continuous(limits = c(0,max(-log10(testdata$q_val_2))), range = c(1,8)) +
    theme_bw() +
    labs(x = '', y = '',
         title = title,
         subtitle = subtitle,
...
testdata_to_st = testdata %>%
    ...
    rename(Country = country,
           ...
           'q-value' = q_val_2,
           'Model' = model_used)
write.csv(testdata_to_st, '../supplementary_tables_regressions/ST16.csv', quote=F, row.names=F)
