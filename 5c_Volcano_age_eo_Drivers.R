...
testdata %>%
    ...
    mutate(
           vartype = str_replace(vartype, 'DR', 'Cancer driver genes'),
           ...
           vartype = factor(vartype, levels = c(
                                                ...
                                                'CNA in driver genes'))) %>%
    filter(vartype %in% c('Cancer driver genes'
                          #, 'Recurrent driver mutations'
                          )) %>%
ggplot() +
...
testdata_to_st = testdata %>%
    ...
    rename('Cancer driver gene' = signature,
        'p-value' = p_val,
        'q-value' = q_val_2,
        'Model' = model_used)

write.csv(testdata_to_st, '../supplementary_tables_regressions/ST22.csv', quote=F, row.names=F)
...
testdata_to_st = testdata %>%
    ...
    arrange(signature) %>%
    select(signature, OR, p_val, q_val_2, model_used) %>%
    rename('Recurrent driver mutation' = signature,
           'p-value' = p_val,
           'q-value' = q_val_2,
           'Model' = model_used)

rdms = testdata_to_st$`Recurrent driver mutation`
extra_info = df %>%
    select(donor_id, all_of(rdms)) %>%
    pivot_longer(cols = all_of(rdms),
                 names_to = 'RDM',
                 values_to = 'Mut_number') %>%
    mutate(Mut = Mut_number>0) %>%
    group_by(RDM) %>%
    summarise(Mut_samples = sum(Mut)) %>%
    rename('Recurrent driver mutation' = RDM,
           'Mutated samples' = Mut_samples)
testdata_to_st = testdata_to_st %>%
    left_join(extra_info) %>%
    select('Recurrent driver mutation', 'Mutated samples',
           OR, 'p-value', 'q-value', 'Model')
    

write.csv(testdata_to_st, '../supplementary_tables_regressions/ST23.csv', quote=F, row.names=F)
