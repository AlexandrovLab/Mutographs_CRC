...
dfplot %>%
    ...
    geom_text(aes(x = ifelse(category == 'APC c.835-8A>G', prev_perc/2,
                             ifelse(category == 'TP53 driver mutations',
                                    0.07,0.12)),
                  label = ifelse(category != 'TP53 driver mutations',
                                 prev_label, '0/110')),
    ...
    labs(x = 'Proportion of mutations probabilistically assigned',
         y = '', fill ='',
         title = 'Signatures assigned to individual single base\nsubstitutions in colibactin positive cases') +
...
dfplot %>%
    ...
    aes(y = reorder(category, desc(category)),
        fill = Signature) +
    geom_bar(position = position_fill(reverse = TRUE)) +
    geom_text(aes(x = prev_perc/2, label = prev_label),
              check_overlap = T, size = 4.5) +
    scale_fill_manual(values = c('skyblue2','grey90', 'grey80', 'grey70',
                                 'grey60',  'grey50')) +
    theme_bw() +
    labs(x = 'Proportion of indels probabilistically assigned',
         y = '', fill ='',
         title = 'Signatures assigned to individual indels\nin colibactin positive cases') +
    scale_x_continuous(labels = label_percent()) +
    theme(plot.title = element_text(size = 16, face = 'bold'),
          plot.subtitle = element_text(size = 16),
          axis.title = element_text(size = 14),
