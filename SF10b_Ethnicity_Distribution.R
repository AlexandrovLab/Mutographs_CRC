...
df %>%
    group_by(ethnicity,country) %>%
    summarise(count = n()) %>%
    left_join(df_country) %>%
    mutate(rel_count = count / all_country,
           col_label = ifelse(rel_count>0.5,
                              'white','black'),
           ethnicity = factor(ethnicity,levels = (
               c(sort(unique(df$ethnicity[df$ethnicity!='Missing'])),
                 'Missing')
           ))) %>%
ggplot() +
    aes(y = reorder(ethnicity, desc(ethnicity)), x = country, fill = rel_count) +
    geom_tile() +
    ...
    theme_bw() +
    labs(x = '', y = '',
         title = 'Self-reported ethnicity distribution by country') +
    guides(col = 'none',
           fill = 'none') +
