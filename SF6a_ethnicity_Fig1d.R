...
met$age_eo = ifelse(met$age_eo == '0-49', '0-49 (n=97)',
                    '50+ (n=705)')
met$ancestry = factor(met$ancestry)
met$ancestry = relevel(met$ancestry, ref = 'EUR')

met$ethnicity[met$ethnicity=='Missing'] = NA
met$ethnicity = factor(met$ethnicity)
met$ethnicity = relevel(met$ethnicity, 'Brazilian')


## TMB SBS
model = lm(log10(TMB_SBS) ~ age_eo + sex + tumorsite_group + ethnicity + purity, data = met)
summary(model)
...
## TMB ID
model = lm(log10(TMB_ID) ~ age_eo + sex + tumorsite_group + ethnicity + purity, data = met)
summary(model)
p_value_adj = summary(model)$coefficients['age_eo50+ (n=705)', 'Pr(>|t|)']
