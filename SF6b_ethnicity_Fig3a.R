...
covdata = df %>% select(donor_id, age_eo, sex, tumorsite_group, ethnicity, purity)
...
testdata <- dfmodel %>%
    ...
    do(tresult = safely(stats::glm)(value ~ age_eo + sex + ethnicity + tumorsite_group + purity,family = binomial(),data=.)) %>% 
...
testdata <- dfmodel %>%
    ...
    group_by(name) %>%
    mutate(value=as.factor(value)) %>% 
    do(tresult = safely(logistf::logistf)(value ~ age_eo + sex + ethnicity + tumorsite_group + purity,
...
testdata %>%
    ...
    labs(y = 'q-value (-log10)',
         ...
         subtitle = '  Adjusted by sex, tumor subsite, purity, and self-reported ethnicity') +
