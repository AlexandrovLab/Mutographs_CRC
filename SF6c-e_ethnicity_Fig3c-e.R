...
df$ethnicity[df$ethnicity=='Missing'] = NA
df$ethnicity = factor(df$ethnicity)
df$ethnicity = relevel(df$ethnicity, 'Brazilian')
...
covdata = df %>% select(donor_id, age_group, sex, tumorsite_group, ethnicity, purity)
...
testdata <- dfmodel %>%
    ...
    do(tresult = safely(stats::glm)(value ~ age_group + sex + tumorsite_group + ethnicity + purity,family = binomial(),data=.)) %>% 
...
testdata <- dfmodel %>%
    ...
    group_by(name) %>%
    mutate(value=as.factor(value)) %>% 
    do(tresult = safely(logistf::logistf)(value ~ age_group + sex + tumorsite_group + ethnicity + purity,
...
dfplot %>%
    ...
    theme_bw() +
    labs(subtitle = 'Adjusted by sex, tumor subsite, purity, and ethnicity',
        x = 'Age of onset',
...
model = (lm(age_diag ~ colibactin_signature_SBS88orID18 + sex +tumorsite_group+ethnicity+purity,
           data = df))
summary(model)
...
df_new %>%
    ...
    labs(x = '',
         ...
         subtitle = 'Adjusted by sex, tumor subsite, purity,\nand ethnicity') +
    theme_bw()+
...
model = (lm(age_diag ~ colibactin_signature_SBS88orID18 + sex +ethnicity + purity,
...
ggplot() +
    ...
    labs(x = '',
         y = 'Age of onset',
         title = NULL,
         subtitle = 'Adjusted by sex, purity, and ethnicity') +
    theme_bw()+
    theme(plot.title = element_text(size = 16, face = 'bold'),
          plot.subtitle = element_text(size = 14, vjust = -22.5),
          axis.title = element_text(size = 14),
          axis.text = element_text(size = 14),
          axis.text.x = element_text(size = 14, angle = 30, hjust = 1),
          legend.title = element_text(size = 16, face ='bold'),
