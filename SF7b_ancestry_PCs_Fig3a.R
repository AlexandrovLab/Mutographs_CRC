...
covdata = df %>% select(donor_id, age_eo, sex, tumorsite_group, purity,
                        PC1, PC2, PC3, PC4, PC5)
...
testdata <- dfmodel %>%
    ...
    do(tresult = safely(stats::glm)(value ~ age_eo + sex + tumorsite_group + purity + PC1 + PC2 + PC3 + PC4 + PC5,family = binomial(),data=.)) %>% 
...
testdata <- dfmodel %>%
    ...
    group_by(name) %>%
    mutate(value=as.factor(value)) %>% 
    do(tresult = safely(logistf::logistf)(value ~ age_eo + sex + tumorsite_group + purity + PC1 + PC2 + PC3 + PC4 + PC5,
                                          data=., control =  logistf.control(maxit = 10000))) %>% 
    mutate(tresult_null = map_lgl(tresult['result'], is.null)) %>% 
    filter(!tresult_null)
...
testdata %>%
    ...
    labs(y = 'q-value (-log10)',
         ...
         subtitle = '  Adjusted by sex, tumor subsite, purity, and genetic ancestry') +
    theme(plot.title = element_text(size = 18),
          plot.subtitle = element_text(size = 14, vjust = -19),
