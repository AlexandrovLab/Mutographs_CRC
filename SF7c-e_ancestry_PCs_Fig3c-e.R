...
covdata = df %>% select(donor_id, age_group, sex, tumorsite_group, ancestry, purity)
...
testdata <- dfmodel %>%
    ...
    do(tresult = safely(stats::glm)(value ~ age_group + sex + tumorsite_group + ancestry + purity,family = binomial(),data=.)) %>% 
...
testdata <- dfmodel %>%
    ...
    group_by(name) %>%
    mutate(value=as.factor(value)) %>% 
    do(tresult = safely(logistf::logistf)(value ~ age_group + sex + tumorsite_group + ancestry + purity,
...
model = (lm(age_diag ~ colibactin_signature_SBS88orID18 + sex +tumorsite_group+ancestry+purity,
           data = df))
summary(model)
...
model = (lm(age_diag ~ colibactin_signature_SBS88orID18 + sex +ancestry + purity,
