...
testdata <- dfmodel %>%
    ...
    do(tresult = safely(stats::glm)(value ~ age_eo + sex + tumorsite_group + purity + ancestry,family = binomial(),data=.)) %>% 
...
testdata <- dfmodel %>%
    ...
    group_by(name) %>%
    mutate(value=as.factor(value)) %>% 
    do(tresult = safely(logistf::logistf)(value ~ age_eo + sex + tumorsite_group + purity + ancestry,
