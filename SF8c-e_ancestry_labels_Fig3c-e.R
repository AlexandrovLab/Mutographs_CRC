...
covdata = df %>% select(donor_id, age_group, sex, tumorsite_group, PC1,PC2,PC3,PC4,PC5, purity)
...
testdata <- dfmodel %>%
    ...
    do(tresult = safely(stats::glm)(value ~ age_group + sex + tumorsite_group + PC1 + PC2 + PC3 + PC4 + PC5 + purity,family = binomial(),data=.)) %>% 
...
testdata <- dfmodel %>%
    ...
    group_by(name) %>%
    mutate(value=as.factor(value)) %>% 
    do(tresult = safely(logistf::logistf)(value ~ age_group + sex + tumorsite_group + PC1 + PC2 + PC3 + PC4 + PC5 + purity,
...
dfplot %>%
    ...
    theme_bw() +
    labs(subtitle = 'Adjusted by sex, tumor subsite, purity, and genetic ancestry',
        x = 'Age of onset',
...
model = (lm(age_diag ~ colibactin_signature_SBS88orID18 + sex +tumorsite_group+PC1 + PC2 + PC3 + PC4 + PC5+purity,
           data = df))
summary(model)
...
model = (lm(age_diag ~ colibactin_signature_SBS88orID18 + sex +PC1 + PC2 + PC3 + PC4 + PC5 + purity,
...
ggplot() +
    ...
    labs(x = '',
         y = 'Age of onset',
         title = NULL,
         subtitle = 'Adjusted by sex, purity, and genetic ancestry') +
    theme_bw()+
    theme(plot.title = element_text(size = 16, face = 'bold'),
